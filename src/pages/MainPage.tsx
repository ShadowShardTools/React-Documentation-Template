...
const MainPage: React.FC = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (!isTyping && (e.code === "Slash" || (e.ctrlKey && e.code === "KeyK"))) {
        e.preventDefault();
  ...
  const navigateToItem = useCallback((item: DocItem, anchor?: string) => {
  ...
  const renderContent = () => {
    ...
    if (loading.content) return <p className="text-gray-500">Loading content...</p>;
    if (error.content) return <ErrorMessage message={error.content} />;
    if (!selectedItem) return <div className="text-gray-500 text-center mt-16">Select a document from the sidebar</div>;

    return (
      <Suspense fallback={<p className="text-gray-400">Loading content…</p>}>
        <ContentRenderer title={selectedItem.title} content={selectedItem.content} />
      </Suspense>
    );
  };
